...
Decision 0040 freezes logical token/position input as canonical prefix evidence.
...
Exactly 59 compact logical-prefix artifacts cover the unique batch/context
inputs across all methods and replicates. They store int32 token IDs plus
identity/checksum controls, never method cache tensors. Every worker validates
the logical input and reconstructs a fresh method cache outside timing. An exact
checksum-valid current-layout Phase 13, Phase 13D, or Phase 16G snapshot may be
restored only as an optional setup acceleration. A readable legacy snapshot
whose layout differs is classified `restore_incompatible` and ignored without
conversion; corrupted input or snapshot evidence fails closed. KVQuant
reconstruction uses the already admitted fixed bounded chunk prefill. No
runtime cache, pointer, or Graph sharing occurs. The Phase 13 stage-timeout
scaling formulas and fixed-L context mapping remain unchanged.
...
to R2 with `COMPLETE` last and a clean retrieval. After that single verified
promotion, compact local indexes and a remote-authoritative receipt are retained
and large local segment staging may be evicted; the locked R2 root remains the
append-only authority. Timing rows retain
