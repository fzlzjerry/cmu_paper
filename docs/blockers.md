...
|---|---|---|---|---|
| B-001 | No Git commit or code SHA exists in the newly initialized repository. | E00 durable gate evidence and every later run | Review Phase 0 files and create the initial commit before E00 produces durable evidence. | open |
...
| B-009 | `/artifacts/` is Git-ignored and no durable append-only store or immutable locator scheme is selected or implemented. | E01 acceptance and any claim-bearing/admission run | Implement and test docs/artifact_policy.md; record the durable store and locator/digest publication mechanism before closing E01. | open |
