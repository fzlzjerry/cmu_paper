#!/usr/bin/env python3
"""Run the narrow Phase 11D checks inside the authorized container."""
...
MANIFEST_RELATIVE = Path(
    "third_party/patches/kvquant/deterministic-long-context-manifest.json"
)
...
class Phase11DValidationError(RuntimeError):
...
def _require_result(record: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise Phase11DValidationError(f"{name} failed")
    return payload
...
def _validate_code_objects(extension: Path) -> dict[str, Any]:
    ...
    for arguments, marker, label in (
        ...
        if not passed:
            raise Phase11DValidationError(f"extension lacks {label}")
...
def _run_sanitizer(
    ...
    if version.returncode != 0:
        raise Phase11DValidationError("Compute Sanitizer version is unavailable")
    records: list[dict[str, Any]] = []
...
def main() -> int:
    ...
    if any(output_root.iterdir()):
        raise Phase11DValidationError("output root must start empty")
    if (
        ...
        raise Phase11DValidationError("authorized container identity differs")
    ...
    if _sha256_file(extension) != extension_sha256:
        raise Phase11DValidationError("fresh extension identity differs")
    ...
    if (
        ...
        raise Phase11DValidationError("source reconstruction validation failed")
    ...
    if (
        ...
        raise Phase11DValidationError("fixture custody validation failed")
    ...
    summary = {
        "schema_version": "kvbench-phase11d-validation-summary-1.0.0",
        "status": "PASS",
